...
export const authRoutes = new Elysia({ prefix: '/user' })
  ...
  .post('/register', async ({ auth_token, body, cookie: { auth }, set }) => {
    try {
      ...
      const [existingUser] = await db.select().from(users).where(or(eq(users.email, body.email), eq(users.username, body.username)));
      if (existingUser) {
        set.status = 400;
        return {
      ...
      if (config.cookieConfig.secure) auth.secure = config.cookieConfig.secure;
      if (config.cookieConfig.sameSite) auth.sameSite = config.cookieConfig.sameSite;
  ...
  .post('/login', async ({ auth_token, body: { email, password }, cookie: { token: auth }, set }) => {
    try {
