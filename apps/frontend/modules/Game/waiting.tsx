...
const WaitingRoom = ({ details, players, onStart, removePlayer }: WaitingRoomProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-screen mb-[120px]">
      <h1 className="text-grey-700 font-extrabold text-[40px] m-0 mt-[5vh] mb-[4vh]">Waiting for more players...</h1>
      <div className="w-1/2 flex flex-col items-center px-7 py-7 pb-5 justify-center bg-white rounded-[14px]">
        <div className="flex flex-row items-center justify-between w-full">
          <div className="text-grey-900 font-bold text-xl self-start text-left">
            No. of Players: <span className="text-grey-600">{ players.length}</span>
          </div>
          <div className="text-grey-900 font-bold text-xl text-center self-center">
            Maximum Players: <span className="text-grey-600">{details.maxPlayers}</span>
          </div>
        ...
        <div className="flex flex-row mt-8 w-full">
          <div className="mr-5 text-grey-900 font-bold text-xl mt-1">Categories: </div>
      ...
      <div className="flex flex-row mt-[4vh] w-4/5 flex-wrap overflow-scroll justify-center">
        {ps.map((player) => {
          return (
            <div className="flex flex-row items-center w-[30%] min-w-[180px] max-w-[240px] px-6 py-4 bg-white rounded-[14px] mx-3.5 my-3.5" key={player.id}>
              ...
              <div>
                <div className="font-extrabold text-xl text-grey-700 overflow-hidden whitespace-nowrap text-ellipsis">{player.username}</div>
      ...
      <div className="flex flex-row items-center justify-between fixed bottom-0 w-screen h-20 bg-white p-5">
        <div className="flex items-center justify-center text-grey-800 font-semibold text-xl ml-[2vw]">
          Game Code:⠀<span className="text-primary font-extrabold">{details.code}</span>
        </div>
